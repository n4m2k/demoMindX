...
const Nam = () => {        
  ...
  useEffect(() => {
    fetch("https://633ae702e02b9b64c61a63ba.mockapi.io/api/v1/quanao/?loai=nam")
    .then((response) => {
  ...
  <div className="container">
          ...
          <div className="col l-2 m-0 c-0">
            <nav className="category">
              ...
              <li classname="category_list_item category_item_active">
                  <a onClick={tatcasanpham} className="category_link">Tất Cả</a>
                </li>
                {
              listDanhmuc ? listDanhmuc.map((item) =>{
                return(
                  <li classname="category_list_item category_item_active">
                  <a onClick={()=>loadSanPham(item)} className="category_link">{item}</a>
                </li>
                )
               }) : ""
            }
      ...
      <div className="col l-10 m-12 c-12">
            ...
            { loading ? (
                <h1>Loading...</h1>) : (
                listData.map((item)=>{
                  return (<div className="grid__column2_4 m-4 c-6">
                      <div className="product-card">
                      <NavLink to={`/product-detail/${item.id}`}  className="product_link">
                      <div className="product-frame">
                        <img src={item.hinhanh} className="product-img" alt="" />
                      </div>
                      <div className="product-caption">
                        <p className="product-name">
                        {item.tensp}
                        </p>
                        <p className="product-price">{item.gia}</p>
                      </div>
